...
final GlobalKey _groupSsKey = GlobalKey();
...
class _GroupVideoCallScreen1State extends State<GroupVideoCallScreen1>
  ...
  Future<void> _captureScreenshot() async {
    try {
      ...
      final boundary = _groupSsKey.currentContext!.findRenderObject()
              ...
              children: [
                Center(
                    child: RepaintBoundary(
                        key: _groupSsKey, child: _remoteGrid(c))),
                ...
                Positioned(
                  ...
                  child: SizedBox(
                    ...
                    child: IconButton(
                      icon: Icon(
                        c.isScreenSharing.value
                            ? Icons.stop_screen_share
                            : Icons.screen_share,
                        color: c.isScreenSharing.value
                            ? Colors.blueAccent
                            : Colors.white,
                      ),
                      onPressed: () {
  ...
  Widget _remoteGrid(GroupVideoCallController c) {
    ...
    final views = c.remoteUids.map((uid) {
      ...
      return muted
          ? Center(
              child: Text(name, style: const TextStyle(color: Colors.white54)),
            )
          : AgoraVideoView(
              controller: VideoViewController.remote(
                rtcEngine: c.engine,
                canvas: VideoCanvas(
                    uid: uid, sourceType: VideoSourceType.videoSourceRemote),
  ...
  Widget _bottomControls(GroupVideoCallController c) {
    ...
    return c.callUIState.value == CallUIState.connected ||
            c.callUIState.value == CallUIState.calling
        ? Positioned(
            bottom: 40,
            left: 0,
            right: 0,
